...
parser.add_argument(
    ...
    type=int,
    default=16,
    help="number of calibration samples for AWQ search",
)
parser.add_argument(
    "--awq_seqlen",
    type=int,
    default=64,
    help="sequence length for AWQ calibration",
)
...
assert (
    args.act_scale_path is not None and args.media_path is not None and len(args.media_path) > 0
...
def build_model_and_enc(model_path, dtype):
    ...
    if vila_10_quant_mode:
        ...
        # Avoid OOM after newer transformers versions
    ...
    if args.load_quant:
        ...
        if args.run_awq:
            ...
            print(
                f"Running AWQ search with n_samples={args.awq_n_samples}, "
                f"seqlen={args.awq_seqlen}"
            )
    ...
    if args.tasks is not None:
        if args.tasks == "wikitext":
            ...
            lm_eval_model = LMEvalAdaptor(
                args.model_path, model, enc, args.batch_size
            )
