...
def main():
    ...
    plt.title("OLMo-1B Base Model: Input vs Output Max Activation per Layer")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()

    plot_path = os.path.join(args.output_dir, "olmo_base_input_output_max.png")
    plt.savefig(plot_path, dpi=200)
    plt.close()

    json_path = os.path.join(args.output_dir, "olmo_base_input_output_max.json")
