...
def load_arc_examples(split: str, limit: Optional[int], subset: str) -> List[Dict]:
    candidate_loaders = [
        lambda: load_dataset("allenai/ai2_arc", subset, split=split),
        lambda: load_dataset("ai2_arc", subset, split=split),
    ]
    ...
    if ds is None:
        raise RuntimeError(f"Could not load ARC dataset. Last error: {last_error}")
        ...
        predictions.append(
            {
                ...
                "choice_labels": ex["choice_labels"],
            }
        )
