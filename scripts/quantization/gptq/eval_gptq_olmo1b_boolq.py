...
def evaluate_boolq(model, tokenizer, limit=500, device="cuda:0"):
    ...
    for i, ex in enumerate(ds):
        ...
        label = 1 if ex["answer"] else 0

        scores = [
            score_choice(model, tokenizer, prompt, c, device)
            for c in choices
        ]
...
if __name__ == "__main__":
    ...
    output_json = Path(
        "results/quantization/olmo1b/gptq/boolq.json"
    )
